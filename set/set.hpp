...
/*   Updated: 2023/03/06 17:35:52 by ael-hayy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	friend class set;
	t_node(Key *v, t_node *par):val(v), left(NULL), right(NULL), parent(par), color(_RED) {}
	Key	* const	val;
	t_node   	*left;
	t_node		*right;
	t_node		*parent;
...
public:
	...
	typedef						set_iterator<const Key, t_node *, Compare>		iterator;
	typedef						set_iterator<const Key, const t_node*, Compare>	const_iterator;
...
//! //////////////////// 
//* CONSTRUCTERS:
	set(): _root(NULL), _size(0) {};
	explicit set( const Compare& comp, const Alloc& alloc = Alloc()):_root(NULL), _size(0), _com(comp), val_alloc(alloc) {}
	template< class InputIt >
	set( InputIt first, InputIt last, const Compare& comp = Compare(), const Alloc& alloc = Alloc() ):_root(NULL), _size(0), _com(comp),  val_alloc(alloc)
	{
		insert(first, last);
	}
	set( const set& other ) : _root(NULL), _size(0)
	{
...
//? DELETE 
	...
	{
		ft::vector<const Key> vec_tem;
		for ( ;first != last; first++)
			vec_tem.push_back(*first);
		for (typename ft::vector<const Key>::iterator i = vec_tem.begin();i != vec_tem.end(); i++)
			this->erase(*i);
	}
...
private:
	...
	t_node	*__allocate_costruct_node(const value_type& v, t_node *par)
	{
		...
		_allocat.construct(saver, t_node(val_alloc.allocate(sizeof(value_type)), par));
		// saver->val = ;
